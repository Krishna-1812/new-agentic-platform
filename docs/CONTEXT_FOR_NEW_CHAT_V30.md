# Intelligence by Position2 - Full Context (v30 - September 2026)

Paste this entire file at the start of a new chat to give the assistant full context on this platform. **v30 supersedes all earlier context files (v1-v29)** - older versions are stale; ignore any pasted copy, and if `CONTEXT_FOR_NEW_CHAT_V29.md` (or older) still exists in the repo root, delete it as part of landing this file per the standing one-canonical-file convention.

**Latest `main` HEAD: `e2f5cea`** (always `git pull` to confirm; Railway auto-deploys every push). `app.py` is **18,571 lines / 224 `@app.route` decorator lines**, up from 18,067 / 203 at v29. The test suite is now **171 files, 4,378 tests collected clean** (`PYTHONDONTWRITEBYTECODE=1 python3 -B -m pytest tests/ -q`; v29 reported 131 files / roughly 3,651 tests), up from 131/3,651 at v29. At least 25 routes carry `@admin_required` (the only count this doc's own test, `test_admin_access.py`, actually asserts is a lower bound; v29's "32" was a point-in-time manual count, treat neither as exact without re-checking).

**What this cycle was:** two independent threads landed on `main` since v29. First, this chat closed out the tail end of the v29 Event & Conference Intelligence cycle (one cosmetic fix, one real bug found and deliberately left open), then ran a full-repository, senior-developer-style audit at the user's explicit request: dependency correctness, a security pass, and a completely rewritten, detailed README. Second, a real GitHub pull request (**#11**, authored by a human teammate, Krishna Ladha, merged outside this chat) landed a genuine hardening pass on the same agent's audit and evidence logic while this chat's audit was in progress; it was pulled into this session's clone via `git merge --ff-only` specifically so this doc could describe the true current state of `main` rather than a stale local view. **Read the "WHAT V30 ADDS" section below before trusting anything under EVENT & CONFERENCE INTELLIGENCE further down this file** - PR #11 changed a rule that section states as a fixed, present-tense invariant.

The transferable lesson this cycle adds to the two already established: **a context doc is itself a cache, and caches go stale the moment someone else pushes.** This session's own local `git log` disagreed with `origin/main` by two commits at the exact moment this doc was being written; the fix was to fetch and fast-forward before writing a single word, not to describe the repo from memory of the last known state.

---

## WHAT V30 ADDS ON TOP OF V29

### 1. Stale run-refusal banner, the last loose end of the v29 cycle (`8abb468`)

Live-browser testing during v29's "make it crazy good" push (full detail in that section below) surfaced a cosmetic bug: `startRun()` only clears the "Complete the client profile before starting" error banner (`formError`) at the top of its OWN next submit attempt. Picking a different, already-complete client from the profile dropdown, or saving a newly-drafted profile (which calls that same `onProfilePick()` on success), left a stale refusal on screen under a button that was actually ready to run. Fixed by clearing `formError` at the top of `onProfilePick()` itself, which covers both paths since `saveProfile()`'s success handler calls it. Two new tests in `tests/test_event_intel_form_init.py`.

**Found in the same browser session, deliberately left open, still open:** `event_intel_policy.intake_errors()` unconditionally requires `buyer_roles` and `verticals` before a "recommend" run can start, for every client regardless of B2C/B2B classification. Confirmed live against two real, independent B2C profiles (Myntra AND Beta Bionics) that this blocks ALL B2C clients from ever running "Score the calendar," not just one client's data being incomplete. This needs a real product decision (exempt B2C profiles from the gate entirely, or reinterpret what the fields mean for a B2C client) before it's touched. Explicitly declined to type fabricated buyer-role/vertical data into a real client's production profile just to force the gate to pass - do not do this either, ask first.

### 2. Full repository audit, requirements.txt fix, README rewrite (`853cc6b`)

User's explicit ask: "a senior developer would access and audit the entire repo," add a requirements file if one is missing, and make the README detailed about every agent.

**Audit method, in case it needs repeating:** AST-parsed every `.py` file's imports and cross-checked the result against `requirements.txt` (a plain text-grep approach would have missed conditional/nested imports); collected the full test suite with `pytest --collect-only` to confirm zero collection errors across the whole tree without spending the time to run all 4,000+ tests; grepped for hardcoded secrets/API-key-shaped strings and PAT/service-token patterns (none found, tracked history included); read every `after_request` hook, the `SECRET_KEY` fallback path, and every `X-Forwarded-For` read site by hand; confirmed CI (`run-tests.yml`) installs `requirements.txt`, runs `pip check`, then the full suite on every PR and push to `main`.

**requirements.txt already existed** (this session fixed it in place, did not create it from nothing):
- Removed `pandas`: zero imports anywhere in the actual application, confirmed by the AST scan above. Only reference anywhere in the repo was a doc file's prose and a completely separate benchmark harness's own pinned `requirements-constraints.txt`. Pure install-time dead weight.
- Added `google-search-results>=2.4.2` (declared with a comment explaining why): `tracker/news_client.py` imports `serpapi.GoogleSearch` for the ABM Signal Tracker's optional paid-news fallback, but the package itself was never listed. It degraded gracefully before this fix (`try/except ImportError`, falls back to the free Google News RSS feed), so nothing in production was actually broken, it was just an undeclared dependency waiting to surprise a fresh `pip install`.

**README.md rewritten** with a full agent catalog: every one of the ten `/p2/b2b-agents/*` agents (route, what it does, vendor/data notes), the SEO Suite's 16 tools named individually, visitor de-anonymization, Vimi, client portals, and admin analytics. Also added an environment-variable reference table and a "Known gaps" section. Cross-checked the agent roster directly against the live `APP_AGENTS`/`AGENTS` registries in `app.py` rather than trusting older docs, since this same doc's own "Agent roster hazard" note (further down) warns the roster drifts across independent lists. **Treat the README itself as the current, canonical short version of the agent catalog** going forward; this doc's agent-by-agent sections below remain the deeper technical/historical reference and the two should stay roughly in sync.

**Security findings, reported but deliberately NOT fixed** (production-behavior-affecting for a live app with real client data; needs explicit go-ahead before implementing, not silent edits):
- No CSRF token anywhere, and no explicit `SESSION_COOKIE_SECURE`/`SESSION_COOKIE_SAMESITE` (relies on Flask/browser defaults).
- Several `X-Forwarded-For` read sites, rate limiting among them, trust the header's first entry with no `ProxyFix`/trusted-proxy validation in front of it, which is spoofable by any client that talks to the app directly.
- No security response headers anywhere: no `Content-Security-Policy`, `X-Content-Type-Options`, or `Strict-Transport-Security`. The only two `after_request` hooks in the app handle HTML cache-control and gzip compression.
- These four line up exactly with this doc's own pre-existing open item ("Advisory security/design audit, do not start without an explicit ask" - see OPEN ITEMS further down). **This audit fulfilled that explicit ask for the REVIEW; it did not authorize the FIXES.** Confirm with the user before implementing any of the above.
- **Also flagged, not fixed, a structural rather than security concern:** `app.py` is a single ~18,500-line, ~970 KB file with no blueprint/package split. Deliberate so far (the module docstring and section-comment banners serve as its map), but it is the first thing a fresh senior reviewer should expect to question as the platform keeps growing, and every one of this session's own line-count deltas above is really just this one file getting bigger again.

### 3. PR #11: audit comparisons stop removing events; organizer structured data becomes real evidence (`e2f5cea`, human teammate Krishna Ladha, merged on GitHub, not built in this chat)

**This changes a rule the EVENT & CONFERENCE INTELLIGENCE section of this doc (below) states as a currently-true, present-tense invariant.** A correction note is inserted inline at that exact bullet ("A famous-event verdict of 'kept' with no named alternative is downgraded to a cut") pointing back to this section; the surrounding historical narrative below it is left otherwise untouched since it accurately describes how the code worked at the time it was written.

- **`audit_famous`'s cut authority was removed.** Before this PR, if the famous-event audit named and confirmed a better alternative for a marquee event, the original event was removed from the results list outright (the old `apply_audit`). Now (`retain_for_scoring`, plus a new `comparison_only` report mode), a named alternative is researched and reported alongside the original, but **neither the original nor the alternative is removed by the audit's own verdict** - both proceed through the identical source-check-and-scoring pipeline, and only the rubric decides what survives. The report now states this framing directly: "Marquee comparisons inform research; they do not remove candidates." Read from the diff and its tests, the likely rationale: the audit's own comparison judgment had been functioning as a second, less-accountable scoring path running outside the rubric it was meant to feed information into.
- **New evidence source: organizer-published schema.org JSON-LD.** `tracker/event_intel_structured.py` (new file, roughly 50 lines, a bounded `HTMLParser`-based JSON-LD extractor with hard caps on how many script blocks and graph nodes it will ever walk) reads an organizer page's own machine-readable `Event` markup (`name`, `startDate`, `endDate`, `eventStatus`, `offers`) wherever `fetch_page` retrieves a page. `event_intel_admission.inspect` now matches this against the exact event name and full date range as a stronger evidence tier (`support: "organizer_structured_name_and_dates"`), and reads `eventStatus`/`offers[].availability` to surface restriction reasons plain rendered-text scanning could never see: a cancelled, postponed, or rescheduled edition, or sold-out/discontinued ticket inventory. Only trusted when the fetch was not truncated and returned a clean HTTP 200, both newly and explicitly checked.
- **Fixed a real ambiguous-date bug.** The admission check's numeric-date matching (`3/3/2026`-style) previously generated and matched BOTH the month-day and day-month interpretation of every numeric date on a page, with nothing distinguishing which one the organizer actually meant. It now requires an explicit `DD/MM` or `MM/DD` marker written elsewhere on the same page, or infers the order unambiguously only when the day component exceeds 12; otherwise it skips numeric-date matching for that page entirely rather than risk confirming against the wrong date.
- **`fetch_page` now records when it truncated a page** (`truncated: True` at the byte cap), which was previously computed and then silently discarded. The new structured-data trust gate above depends on this flag existing.
- **`is_committed_same_edition` now also requires matching year(s) parsed out of the name**, not just name-and-region match, so two different-year editions of the same series (say, "Acme Summit 2026" vs. "Acme Summit 2027") are no longer treated as the identical already-committed edition.
- **The client-intake search fallback now discards a parsed draft that cites zero sources**, closing a gap adjacent to the one `c185e30`/`7fbcf80` already closed on the same code path in the v29 cycle.
- **Minor:** the report drawer's "Download full research (JSON)" link now carries the HTML `download` attribute, so it saves the file natively instead of navigating the browser tab to the raw JSON.
- New/updated tests: `tests/test_event_intel_review_regressions.py` (new file) plus targeted updates across most of the existing Event & Conference Intelligence test files; `test_event_intel_store_postgres.py` gained real-Postgres coverage for the new comparison-preserving behavior specifically (one of the PR's three sub-commits was titled "Verify advisory comparisons in PostgreSQL integration tests").

The v29 material directly below (its own three-thread summary, and everything under it) is left in place as history, not because it is still the current cycle.
...
- **SEO Studio (embedded SEO tools, Node + React/Vite, SEPARATE Railway service):** now lives in this repo at `seo-apps/` (imported 2026-09-24 from `ai-positon2/seo-apps`, history kept; all 22 original branches in `archive/seo-apps-all-branches.bundle`). Built from `seo-apps/Dockerfile` (`seo-apps/railway.json`); its URL is the platform's `SEO_STUDIO_URL`, and both services share `SEO_STUDIO_SECRET`.
...
- **A famous-event verdict of "kept" with no named alternative is downgraded to a cut**, because the comparison that step requires never actually happened. **[SUPERSEDED at v30 by PR #11 / `e2f5cea` - see "WHAT V30 ADDS" near the top of this file. The audit no longer cuts events on its own verdict at all; a named alternative is researched and reported, and only the rubric's own scoring decides what survives. This bullet is kept as an accurate record of the original v28 design, not of current behavior.]**
...
**Do not confuse** this (your own engagement data from a Sheet) with: **LinkedIn Strategy Researcher** (the new NATIVE Arena-backed competitive-strategy agent, its own section above), **LinkedIn Social Researcher** (the OLD external tool, currently hidden), the ABM Signal Tracker's own News Mention/Partnership categories, Social Media Intelligence (creative/vision analysis of organic posts across 6 platforms, an entirely different agent), or Job Change Alert (new-role detections from Slack).
...
│                                      ← Social Media Intelligence, both vendors
...
│   │       social_media_intelligence.html, event_conference_intelligence.html
...
29. **Advisory security/design audit: the REVIEW happened at v30 (the user's full-repo audit ask), the FIXES are still gated on a separate explicit ask.** Confirmed findings: no CSRF token, no explicit `SESSION_COOKIE_SECURE`/`SAMESITE`, `X-Forwarded-For` trusted with no `ProxyFix`, no security response headers (CSP/HSTS/`X-Content-Type-Options`). `SECRET_KEY`/`GOOGLE_CLIENT_ID` fail-closed behavior and cookie flags specifically were checked and are as described in this doc's RUNNING/ENVIRONMENT sections; CSS token convergence and accessibility were NOT part of the v30 audit's scope and remain untouched open items.
