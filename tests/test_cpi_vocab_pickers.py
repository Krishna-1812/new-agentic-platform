...
def test_the_endpoint_serves_each_vocabulary(client):
    for kind in av.kinds():
        r = client.get("/strategic-agents/company-people-intelligence/vocab?kind=%s" % kind)
...
def test_the_endpoint_refuses_a_vocabulary_it_does_not_have(client):
    r = client.get("/strategic-agents/company-people-intelligence/vocab?kind=colour")
    assert r.status_code == 400
    assert "naics" in r.get_json()["kinds"]


def test_the_endpoint_returns_the_format_hint_for_a_code_vocabulary(client):
    r = client.get("/strategic-agents/company-people-intelligence/vocab?kind=naics")
    assert "6 digits" in r.get_json()["hint"]
    r = client.get("/strategic-agents/company-people-intelligence/vocab?kind=technology")
...
def test_the_endpoint_costs_nothing_and_calls_apollo_for_nothing(client, monkeypatch):
    ...
    for kind in av.kinds():
        r = client.get("/strategic-agents/company-people-intelligence/vocab"
                       "?kind=%s&q=a" % kind)
        assert r.status_code == 200
...
def test_the_page_tells_the_browser_where_the_vocabulary_endpoint_is(client):
    r = client.get("/strategic-agents/company-people-intelligence")
    assert r.status_code == 200
