"""/p2/strategic-agents/social-media-intelligence (page + analyze/status/run).
...
def test_page_requires_login():
    c = appmod.app.test_client()
    resp = c.get("/p2/strategic-agents/social-media-intelligence", follow_redirects=False)
...
def test_analyze_requires_login():
    c = appmod.app.test_client()
    resp = c.post("/p2/strategic-agents/social-media-intelligence/analyze",
...
def test_search_returns_companies_from_sci_company_search(monkeypatch):
    ...
    resp = _client().get("/p2/strategic-agents/social-media-intelligence/search?q=Acme")
    assert resp.status_code == 200
    body = resp.get_json()
...
def test_search_with_no_query_returns_an_empty_list_without_calling_apollo(monkeypatch):
    ...
    resp = _client().get("/p2/strategic-agents/social-media-intelligence/search")
...
def test_search_degrades_to_an_empty_list_without_a_configured_key(monkeypatch):
    ...
    resp = _client().get("/p2/strategic-agents/social-media-intelligence/search?q=apple")
    assert resp.status_code == 200
    assert resp.get_json()["companies"] == []
...
def test_a_failed_search_returns_a_reason_not_a_bare_empty_list(monkeypatch):
    ...
    body = _client().get("/p2/strategic-agents/social-media-intelligence/search?q=apple").get_json()
...
def test_a_failed_search_falls_back_to_the_users_own_analyzed_companies(monkeypatch):
    ...
    body = _client().get("/p2/strategic-agents/social-media-intelligence/search?q=goo").get_json()
...
def test_a_successful_search_never_consults_history(monkeypatch):
    ...
    body = _client().get("/p2/strategic-agents/social-media-intelligence/search?q=Acme").get_json()
    assert not called
...
def test_a_genuine_zero_result_carries_no_error_at_all(monkeypatch):
    ...
    body = _client().get("/p2/strategic-agents/social-media-intelligence/search?q=zzz").get_json()
    assert body == {"companies": []}


def test_search_requires_login():
    c = appmod.app.test_client()
    resp = c.get("/p2/strategic-agents/social-media-intelligence/search?q=Acme", follow_redirects=False)
    assert resp.status_code in (302, 401, 403)
...
def test_analyze_requires_a_company_name(monkeypatch):
    resp = _client().post("/p2/strategic-agents/social-media-intelligence/analyze", json={})
...
def test_analyze_starts_running_even_without_any_vendor_keys_configured(monkeypatch):
    ...
    resp = _client().post("/p2/strategic-agents/social-media-intelligence/analyze",
...
def test_run_status_404s_for_a_run_that_belongs_to_someone_else(monkeypatch):
    ...
    resp = _client(_OWNER).get("/p2/strategic-agents/social-media-intelligence/runs/1/status")
...
def test_run_status_404s_for_a_nonexistent_run(monkeypatch):
    ...
    resp = _client(_OWNER).get("/p2/strategic-agents/social-media-intelligence/runs/999/status")
    assert resp.status_code == 404
...
def test_run_detail_404s_for_a_run_that_belongs_to_someone_else(monkeypatch):
    _owner_scoped_get_run(monkeypatch, _run(run_id=1, email=_OTHER))
    resp = _client(_OWNER).get("/p2/strategic-agents/social-media-intelligence/runs/1")
    assert resp.status_code == 404
...
def test_history_page_applies_stale_run_resolution(monkeypatch):
    ...
    _always_resolves_to_error(monkeypatch)
    resp = _client(_OWNER).get("/p2/strategic-agents/social-media-intelligence")
    assert resp.status_code == 200
