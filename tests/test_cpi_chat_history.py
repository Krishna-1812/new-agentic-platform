...
def test_a_disambiguation_prompt_is_not_saved(saved, monkeypatch):
    ...
    with c.session_transaction() as sess:
        sess["google_user"] = {"email": "reporting@markifydigital.com", "name": "T"}
    r = c.post("/strategic-agents/company-people-intelligence/chat",
...
def test_an_empty_question_saves_nothing(saved, monkeypatch):
    ...
    with c.session_transaction() as sess:
        sess["google_user"] = {"email": "reporting@markifydigital.com", "name": "T"}
    c.post("/strategic-agents/company-people-intelligence/chat", json={"message": "   "})
    assert saved == []
...
def _enrich(monkeypatch, profile, kind="person"):
    ...
    with c.session_transaction() as sess:
        sess["google_user"] = {"email": "reporting@markifydigital.com", "name": "T"}
    r = c.post("/strategic-agents/company-people-intelligence/enrich",
