"""ABM Signal Tracker's URLs moved: /accounts -> /abm-signal-tracker/accounts,
and /signal-tracker/<account> -> /abm-signal-tracker/<account>.

Three URL generations exist for the dashboard path alone (bare /signal-tracker/<id>,
then /signal-tracker/<id> after the v16 /p2 relocation, now /abm-signal-tracker/<id>),
...
def test_new_accounts_url_serves_the_picker_directly():
    ...
    with c.session_transaction() as sess:
        sess["google_user"] = {"email": "reporting@markifydigital.com", "name": "T"}
    resp = c.get("/abm-signal-tracker/accounts")
    assert resp.status_code == 200


def test_new_dashboard_url_serves_a_known_account_directly(client):
    resp = client.get("/abm-signal-tracker/healthcare")
    assert resp.status_code == 200


def test_new_dashboard_url_still_404s_an_unknown_account(client):
    resp = client.get("/abm-signal-tracker/not-a-real-account")
...
def test_old_p2_accounts_redirects_straight_to_canonical(client):
    ...
    assert resp.headers["Location"].endswith("/abm-signal-tracker/accounts")


def test_bare_accounts_redirects_straight_to_canonical_not_through_old_p2_path():
    """Before this rename, bare /accounts bounced once through /accounts. Now that
    /accounts is itself a redirect, /accounts must skip straight to the final URL
...
def test_old_p2_signal_tracker_redirects_straight_to_canonical(client):
    resp = client.get("/signal-tracker/healthcare", follow_redirects=False)
    assert resp.status_code == 301
    assert resp.headers["Location"].endswith("/abm-signal-tracker/healthcare")


def test_old_p2_signal_tracker_section_url_preserves_the_section(client):
    resp = client.get("/signal-tracker/healthcare/pipeline", follow_redirects=False)
    assert resp.status_code == 301
    assert resp.headers["Location"].endswith("/abm-signal-tracker/healthcare/pipeline")
...
def test_dashboard_curacct_regex_recognizes_the_new_canonical_path(report_file):
    ...
    (dashboard, signal-tracker) -- on the real, now-served /abm-signal-tracker/<id>
