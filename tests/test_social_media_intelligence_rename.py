...
def _rendered_page():
    ...
    r = c.get("/strategic-agents/%s" % _SLUG)
...
def test_every_template_carrying_the_roster_names_it_the_same_way():
    ...
    m = re.search(r'"t":\s*"([^"]*)",\s*"d":\s*"([^"]*)",\s*'
                  r'"u":\s*"/strategic-agents/%s"' % _SLUG, shared)
    ...
    for base, dirs, files in os.walk(os.path.join(_ROOT, "templates")):
        for name in sorted(files):
            ...
            for c in re.finditer(r"\{t:'([^']*)',d:'([^']*)',u:'/strategic-agents/%s'" % _SLUG, text):
...
def test_the_new_url_serves_the_page():
    ...
    assert "/strategic-agents/%s" % _SLUG in rules


def test_the_old_page_url_permanently_redirects_to_the_new_one():
    c = appmod.app.test_client()
    r = c.get("/strategic-agents/%s" % _OLD_SLUG, follow_redirects=False)
    assert r.status_code == 301
    assert r.headers["Location"].endswith("/strategic-agents/%s" % _SLUG)


def test_the_redirect_shim_carries_no_auth_decorator_of_its_own():
    """Same rule as the nine legacy /admin/* shims: the gate answers at
...
def test_the_four_api_paths_answer_on_both_slugs():
    ...
    for tail in ("/search", "/analyze", "/runs/<int:run_id>/status", "/runs/<int:run_id>"):
        for slug in (_SLUG, _OLD_SLUG):
            assert "/strategic-agents/%s%s" % (slug, tail) in rules, (slug, tail)


def test_the_api_paths_are_not_redirects():
    """If the old API paths were shims, /analyze would break on POST."""
    c = appmod.app.test_client()
    r = c.post("/strategic-agents/%s/analyze" % _OLD_SLUG, json={}, follow_redirects=False)
    assert r.status_code != 301


def test_the_page_javascript_calls_the_new_paths():
    page = _read("templates", "social_media_intelligence.html")
    assert "var BASE = '/strategic-agents/%s'" % _SLUG in page
...
def test_page_view_history_folds_on_the_path_axis_too():
    ...
    # then was recorded under the section's name AT THE TIME: /b2b-agents,
    # not /strategic-agents. The fold has to cross both renames at once.
    old = "/p2/b2b-agents/%s" % _OLD_SLUG      # as recorded: with the old /p2 prefix
    new = "/strategic-agents/%s" % _SLUG
