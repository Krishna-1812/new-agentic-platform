...
def test_export_csv_has_headers_and_bom(client):
    r = client.post("/strategic-agents/company-people-intelligence/export",
...
def test_bulk_enrich_is_capped_and_deduped(client, monkeypatch, no_postgres):
    ...
    r = client.post("/strategic-agents/company-people-intelligence/enrich-bulk", json={"ids": ids})
    assert r.status_code == 200
    body = r.get_json()
...
def test_bulk_enrich_reports_cache_hits_separately(client, monkeypatch):
    ...
    body = client.post("/strategic-agents/company-people-intelligence/enrich-bulk",
...
def test_history_degrades_without_postgres(client, no_postgres):
    body = client.get("/strategic-agents/company-people-intelligence/history").get_json()
    assert body == {"entries": [], "available": False}


def test_history_entry_404s_without_postgres(client, no_postgres):
    assert client.get("/strategic-agents/company-people-intelligence/history/1").status_code == 404
...
def test_paging_grows_the_existing_history_entry(fake_pg, client):
    ...
    body = client.post("/strategic-agents/company-people-intelligence/history",
...
def test_history_save_scopes_every_write_to_the_signed_in_user(fake_pg, client):
    ...
    client.post("/strategic-agents/company-people-intelligence/history",
                json={"entity": "people", "filters": {}, "rows": [{"id": "p1"}],
                      "replace_id": 7})
...
def test_capped_is_only_true_when_rows_were_actually_dropped(client, no_postgres, monkeypatch):
    ...
    def _post(n):
        return client.post("/strategic-agents/company-people-intelligence/enrich-bulk",
...
def test_a_plain_company_name_resolves_to_organization_ids(
    ...
    r = client.post("/strategic-agents/company-people-intelligence/search",
...
def test_company_name_resolution_is_cached_across_pages(
    ...
    r1 = client.post("/strategic-agents/company-people-intelligence/search",
                     json={"entity": "people", "filters": {"company_domains": ["Position2"]},
                           "page": 1})
    r2 = client.post("/strategic-agents/company-people-intelligence/search",
                     json={"entity": "people", "filters": {"company_domains": ["Position2"]},
                           "page": 2})
...
def test_a_not_found_company_name_is_never_cached(
    ...
    miss = client.post("/strategic-agents/company-people-intelligence/search",
                       json=_cpi_search_body(company_domains=["Position2"]))
    hit = client.post("/strategic-agents/company-people-intelligence/search",
                      json=_cpi_search_body(company_domains=["Position2"]))
...
def test_ai_explanation_is_honest_when_nobody_at_all_is_on_file(
    ...
    monkeypatch.setattr(appmod, "_cpi_grounded_answer", _fake_answer)
    client.post("/strategic-agents/company-people-intelligence/search",
               json={"entity": "people",
                     "filters": {"titles": ["CMO"], "company_domains": ["olacabs.com"]}})
