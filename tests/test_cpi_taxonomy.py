...
def test_the_picker_endpoint_answers_a_partial_word(client):
    r = client.get("/strategic-agents/company-people-intelligence/industries?q=heal")
...
def test_the_picker_endpoint_needs_no_apollo_key(client, monkeypatch):
    ...
    r = client.get("/strategic-agents/company-people-intelligence/industries")
    assert r.status_code == 200
...
def test_the_picker_endpoint_is_staff_only():
    ...
    r = anon.get("/strategic-agents/company-people-intelligence/industries?q=heal")
    assert r.status_code in (301, 302, 401, 403)


def test_an_overlong_query_is_truncated_not_rejected(client):
    r = client.get("/strategic-agents/company-people-intelligence/industries?q="
                   + "a" * 500)
    assert r.status_code == 200
