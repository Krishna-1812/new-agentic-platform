"""/strategic-agents/job-change-alert (page + /data + /sync). The page itself is
...
def test_page_renders_for_any_position2_staff(seeded_db):
    resp = _client("someone@markifydigital.com").get("/strategic-agents/job-change-alert")
    assert resp.status_code == 200
    assert b"Job Change Alert" in resp.data


def test_data_endpoint_returns_the_seeded_event(seeded_db):
    resp = _client().get("/strategic-agents/job-change-alert/data")
    body = resp.get_json()
    assert body["total"] == 1
...
def test_sync_route_is_forbidden_for_non_admin_position2_staff(seeded_db):
    resp = _client("someone@markifydigital.com").post("/strategic-agents/job-change-alert/sync")
    assert resp.status_code == 403


def test_sync_route_requires_login():
    c = appmod.app.test_client()
    resp = c.post("/strategic-agents/job-change-alert/sync", follow_redirects=False)
...
def test_sync_route_degrades_gracefully_without_a_slack_token(seeded_db, monkeypatch):
    ...
    resp = _client().post("/strategic-agents/job-change-alert/sync")
    assert resp.status_code == 200
    body = resp.get_json()
