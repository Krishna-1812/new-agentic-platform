...
// Architect's own store has no competitor tracking otherwise, so this is the
// standalone tool's only source of real competitor domain names.
...
// for the reasoning). Cached onto the stored analysis so re-opening a cluster
// doesn't re-spend; re-running is an explicit re-click, not a page-load side
// effect.
router.post('/projects/:id/clusters/:clusterId/suggest-spokes', async (req, res) => {
  try {
    ...
    // local (no network calls), and the stored pages already carry every
    // field it reads (title/h1/h2s/metaDescription/firstParagraph/url), so
    // this reproduces the exact profiles the original analysis used without
    ...
    // cluster's own mechanical top-2-terms name.
