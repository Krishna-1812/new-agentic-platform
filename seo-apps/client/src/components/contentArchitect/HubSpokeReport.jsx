...
// and for either case, what NEW spoke topics would round it out (content the
// site hasn't published yet, not a page it forgot to link to).
//
...
// itself: it only reads `step` (one status line, not the full badge
...
function InlineKeywordResearch({ topic, client, navigate }) {
  ...
  // enforces — one brief targets a primary keyword pair, not an open-ended
  // list. Unchecking is never blocked, only adding a 3rd.
  function toggle(kw) {
    setApproved((prev) => {
      const k = keyOf(kw);
      if (prev.has(k)) {
...
function SpokeTable({ spokes }) {
  const columns = 'minmax(0,1fr) 90px 100px';
  return (
    <div>
      <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '.05em', color: 'var(--text-3)' }}>
...
export default function HubSpokeReport({ analysis, pageById, navigate, projectId, siteName, onSuggestions }) {
  ...
  // Needs attention first, worst health first.
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <p style={{ margin: 0, fontSize: 15, lineHeight: 1.5, color: 'var(--text-2)', maxWidth: '70ch' }}>
          ...
          pages. Here&rsquo;s where that pattern is working, where a topic has spokes but no hub to
          tie them together, and what new spoke topics would round each hub out.
        </p>
      </div>
