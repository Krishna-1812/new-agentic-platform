...
@position2_required
def hub():
    # Same "count it, don't type it" rule the other two hero cards use for
    # their own numbers: the "N dashboards across all three" figure below
    # folds in every live Company Signal Tracker dashboard from ACCOUNTS
...
#
# Unlike the Job Change Alert roster above, this sheet IS shared with the
# platform's service account, so tracker.slot_checker.fetch() reads it live
# (tabs "LPs" + "Locations") on every cache miss. A committed fallback
# snapshot (data/slot_checker_snapshot.json) is still read if that live read
...
def _slot_checker_data_json(force):
    ...
    800 bytes when the client accepts it, so doing it again in the view would
    only add a second cache to invalidate. slot_checker.fetch() is itself
    TTL-cached, so the cost of recompressing per request is a compression of
...
def _sci_vision_claude_url_selftest() -> dict:
    ...
    still failing on every real post if the vendor's own server-side fetch
    of a link (or this repo's local retry of it) is what is broken."""
    from tracker import sci_vision
    try:
        return sci_vision.probe_url()
    except Exception as e:
        return {"configured": False, "error": "%s: %s" % (type(e).__name__, str(e)[:300])}
...
def _dominant_ads_currency(rows) -> str:
    """The converted-currency code most rows agree on (typically all of
    them) -- majority vote rather than "the first row's" so one stray
    unconverted row can't flip the whole page's currency symbol."""
    counts: dict = {}
    for r in rows:
        if r["currency"]:
